...
const ProfanitySelectedEditor = ({ editorId }) => {
  ...
  if (firstLoad) {
      axios.get(`${process.env.VITE_SERVER_CONNECTION_URL}/get_profanity_data?editor_id=${editorId}`).then(res => {  
  ...
  const removeWord = async (word) => {
    if (!word || word.length <= 0) return;
    await axios.post(`${process.env.VITE_SERVER_CONNECTION_URL}/remove_profanity_word`, { word: word, editorId: editorId }).then(res => {
        window.location.reload(false);
    });
  }
