...
const App = () => {
  ...
  const sendMessage = async (agentName) => {
    ...
    axios.post(`${process.env.VITE_SERVER_CONNECTION_URL}/execute`, body).then(res => {
      doCORSRequest(`https://en.wikipedia.org/w/api.php?action=query&format=json&formatversion=2&prop=pageimages&origin=*&piprop=original&titles=${res.data.keyword}`, (data) => { 
